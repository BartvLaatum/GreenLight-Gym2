...
SX update(const SX& x, const SX& u, const SX& d, const SX& p) {
    ...
    */
    ...
    //a[24] = tau12(a.tauShScrShScrPerFir, p.tauRfFir, a[198]hoShScrShScrPerFirDn, p(67))
    a[24] = p(70);

    //FIR reflection coefficient of the cover, excluding screens and lamps [-]
    //a[25] = rhoUp(a.tauShScrShScrPerFir, a[198]hoShScrShScrPerFirUp, a[198]hoShScrShScrPerFirDn, p(67))
    a[25] = p(67);
    ...
    //Fraction going up and absorbed is (1-p(190))*(1-exp(-p(200)*(1-p(189))*gl.a[31]))
    //Fraction going down and absorbed is p(190)*(1-exp(-p(200)*p(189)*gl.a[31]))
    //This is their sum
    //e.g., if p(189)==1, the lamp is above the canopy
    //  fraction going up and abosrbed is 0
    //  fraction going down and absroebd is p(190)*(1-exp(-p(200)*gl.a[31]))
    ...
    //Equation 28 [1]
    //addAux(gl, 'rParSunFlrCanUp', mulNoBracks(gl.a[39], exp(-p(32)*gl.a[31])*p(98)* 
    ...
    //    exp(-p(35)*gl.a[31]), x.tPipe, x.tCovIn))
    ...
    //    exp(-p(35)*gl.a[31]), x.tPipe, x.tThScr))
    ...
    //    exp(-p(35)*gl.a[31]), x.tFlr, x.tThScr))
    ...
    //    exp(-p(35)*gl.a[31]), x.tFlr, x.tBlScr))
    ...
    a[132] = u(3) * p(55) * a[130] / (2. * p(46)) * 
        sqrt(fabs(p(26) * p(56) * (x(2) - d(1)) / (2. * (0.5 * x(2) + 0.5 * d(1) + 273.15)) + a[131] * (d(4) * d(4))));
    ...
    //addAux(gl, 'fVentRoof', ifElse([getDefStr(gl.a[127]) '>=p(8)'], p(57)*gl.a[132]+p(204)*gl.a[135],
    ...
    a[142] = u(2) * p(84) * pow(fabs(x(2) - x(3) + 1e-10), 0.66) + 
        ((1. - u(2)) / a[141]) * sqrt(0.5 * a[141] * (1. - u(2)) * p(26) * fabs(a[140] - a[139]) + 1e-10);
    ...
    a[143] = u(5) * p(94) * pow(fabs(x(2) - x(3) + 1e-10), 0.66) + 
        ((1. - u(5)) / a[141]) * sqrt(0.5 * a[141] * (1. - u(5)) * p(26) * fabs(a[140] - a[139]) + 1e-10);
    ...
    a[147] = if_else(
        x(8) > x(2),
        sensible(1.7 * pow(fabs(x(8) - x(2) + 1e-10), (1./3.)), x(2), x(8)),
        sensible(1.3 * pow(fabs(x(2) - x(8) + 1e-10), (1./4.)), x(2), x(8))
    );
    ...
    //    x.tAir,x.tThScr))
    a[148] = sensible(1.7 * u(2) * pow(fabs(x(2) - x(7) + 1e-10), (1./3.)), x(2), x(7));
    ...
    a[149] = sensible(1.7 * u(5) * pow(fabs(x(2) - x(20) + 1e-10), (1./3.)), x(2), x(20));
...
//    x.tAir, d.tOut))
    ...
    //    x.tPipe, x.tAir))
    a[157] = sensible(1.99 * M_PI * p(105) * p(107) * pow(fabs(x(9) - x(2) + 1e-10), 0.32), x(9), x(2));
    ...
    //    x.tGroPipe, x.tAir))
    a[166] = sensible(1.99 * M_PI * p(167) * p(166) * pow(fabs(x(19) - x(2) + 1e-10), 0.32), x(19), x(2));
    ...
    a[181] = cond(1.7 * u(2) * pow(fabs(x(2) - x(7) + 1e-10), (1./3.)), x(15), satVP(x(7)));
    ...
    a[182] = cond(1.7 * u(5) * pow(fabs(x(2) - x(20) + 1e-10), (1./3.)), x(15), satVP(x(20)));
    ...
    a[183] = cond(p(50)* pow(fabs(x(3) - x(5) + 1e-10), (1./3.)) * p(47)/p(46), x(16), satVP(x(5)));
...
//    d.vpOut, x.tAir, d.tOut))
    ...
    //     sqrt(pow((a[195] + p(137) * a[191]), 2) - 4*p(136) * a[195] * p(137) * a[191]));
    a[196] = (1. / (2. * p(136))) * (a[195] + p(137) * a[191] -
        sqrt(pow((a[195] + p(137) * a[191]), 2) - 4*p(136) * a[195] * p(137) * a[191] + 1e-10));
    ...
    //addAux(gl, 'mcTopOut', airMc(gl.a[136], x.co2Top, d.co2Out))
